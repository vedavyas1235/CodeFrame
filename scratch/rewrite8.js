...
// Find body
const sigStart = code.indexOf('export async function renderFrameSteppedMp4(');
const bodyStart = code.indexOf('  const encoder = await createMp4FrameEncoder', sigStart);
const bodyEnd = code.indexOf('}\n\n', bodyStart) + 2;

if (bodyStart !== -1 && bodyEnd > bodyStart) {
  const newBody = `  // Dynamic Hardware Scaling
...
`;
  code = code.substring(0, bodyStart) + newBody + code.substring(bodyEnd);
